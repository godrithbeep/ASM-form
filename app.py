...
def invalid_mask(df: pd.DataFrame):
    return (df["Khách hàng (C1)"] == "") | (df["Sản phẩm"] == "")
...
def export_excel(user: dict, detail_df: pd.DataFrame, overview_df: pd.DataFrame, crop_df: pd.DataFrame) -> bytes:
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        # Sheet 3: Cây trồng (mirrors original template layout)
        cdf = crop_df.copy()
...
def login_screen():
    ...
    with st.form("login_form", clear_on_submit=False):
        code = st.text_input("Mã nhân viên", placeholder="VD: E1082")
    ...
    if submitted:
        ...
        if match.empty:
            ...
            st.rerun()
    st.markdown("---")
    st.caption("Mã ví dụ để demo: `E1082`, `E0691`, `E0943` (mỗi mã ứng với một Vùng khác nhau).")
...
def step1_screen():
    ...
    st.caption(
        "Bắt buộc: **Khách hàng** và **Sản phẩm**. Đơn giá tự lấy theo sản phẩm (vùng "
        f"**{region}**). Số lượng nhập theo **thùng**. Tỉnh tự đồng bộ theo Khách hàng đã chọn trước đó."
    )
    ...
    column_config = {
        "Khách hàng (C1)": st.column_config.TextColumn("Khách hàng (C1)", required=True, width="medium"),
...
def step2_screen():
    ...
    column_config = {
        ...
        "Target VNĐ": st.column_config.NumberColumn("Target VNĐ", format="%d", disabled=True),
    ...
    def highlight_total(row):
        ok = abs(row["Tổng %"] - 100) < 1e-6
        color = "background-color:#e7efe3" if ok else "background-color:#fbe9e5"
        return [color if col == "Tổng %" else "" for col in row.index]

    st.markdown("**Xem trước trạng thái từng dòng:**")
    st.dataframe(
        edited.style.apply(highlight_total, axis=1).format({"Kg/L": "{:.1f}", "Target VNĐ": fmt_vnd, "Tổng %": "{:.1f}%"}),
